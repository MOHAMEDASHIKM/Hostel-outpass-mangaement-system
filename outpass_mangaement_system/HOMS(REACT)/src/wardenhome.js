        ...
        <ul>
          {outpasses.map((outpasses) => (
            <li key={outpasses._id} className="notelist-item">
              ...
              <div id='btspace'>
                <button id='rejectbtn' onClick={() => { setStatus("rejected  ❌"); setAproved(false) ;statusUpdate(outpasses._id) }}>X</button>

                <button id='selectbtn' onClick={() => { setStatus("Approved  ✅"); setAproved(true) ; statusUpdate(outpasses._id) }}>✅︎</button>
              </div>
